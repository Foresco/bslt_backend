...
(
  ...
  SUM(a.quantity_ratio*a.quantity)/1000 AS norm -- Норма (Исходные нормы на 1000)
  FROM (
    SELECT l.id, l.child_id, l.parent_id, la.quantity AS parent_quantity, la.quantity_ratio, l.quantity
    FROM (
      SELECT a.child_id, SUM(a.quantity) AS quantity, SUM(a.quantity_ratio) AS quantity_ratio
      FROM fn_linked_all(%(object_id)s, %(quantity)s, NULL) a
      GROUP BY a.child_id
    ) la
...
LEFT JOIN (
  SELECT c.child_id, STRING_AGG(e.code, ', ') AS parent_code
  FROM dbo.fn_all_parents_dse(%(object_id)s, 1, NULL) c
  INNER JOIN core_entity e ON (e.id = c.dse_id)
  GROUP BY c.child_id
) p ON (p.child_id = b.child_id)
ORDER BY cc.order_num, group_code, ms.code, IIF(b.replaced_id = 0, 0, 1), mater_code;