...
BEGIN
  ...
  SELECT lt.child_id, l.child_id, lt.quantity*l.quantity*@quantity, lt.quantity*l.quantity*@quantity, @a_level
  FROM core_link lt
  INNER JOIN core_link l ON (l.parent_id = lt.child_id)
  WHERE (lt.parent_id = @object_id) AND (lt.dlt_sess = 0) AND (l.dlt_sess = 0)
